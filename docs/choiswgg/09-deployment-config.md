...
| `EC2_SSH_PRIVATE_KEY` | 배포 전용 SSH **개인키** 전체 내용 | 아래 "최초 1회 설정" 참고. 로그인용 `.pem` 키를 그대로 쓰지 말고 배포 전용 키를 새로 만들 것 |
...
3. **개인키**(`deploy_key`) 내용 전체를 `EC2_SSH_PRIVATE_KEY` GitHub Secret에 등록 (`be`, `python` 두 리포 다)
