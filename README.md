...
| `admin` | 신고 처리, 회원 목록 조회/상태 변경(정지·복원·탈퇴) 등 관리자 액션 |
...
- **`mysql` 명령을 찾을 수 없음**: Windows에는 MariaDB를 설치해도 `mysql` CLI가 PATH에 안 잡혀 있는 경우가 흔합니다. 아래 중 편한 방법을 쓰세요.
  - DBeaver, HeidiSQL 등 GUI 툴에서 `dummy_data.sql`/`delete_all_tables.sql` 파일을 열어 전체 실행
  - Docker가 있다면 컨테이너의 mysql 클라이언트로 로컬 MariaDB에 접속: `docker run --rm -i mariadb:10.6 mysql -h host.docker.internal -P 3306 -u root -p<비밀번호> sangsangseoga < src/main/resources/docs/sql/dummy_data.sql`
  - Python이 있다면 `pip install pymysql` 후 파일을 읽어 세미콜론 기준으로 분리 실행(단, `--` 로 시작하는 주석 줄은 먼저 제거해야 함)
