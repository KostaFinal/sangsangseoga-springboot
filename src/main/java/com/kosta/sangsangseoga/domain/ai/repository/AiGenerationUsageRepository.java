...
public interface AiGenerationUsageRepository extends JpaRepository<AiGenerationUsage, Long> {
    ...
    @Query("SELECT a FROM AiGenerationUsage a JOIN FETCH a.member WHERE a.createdAt >= :from")
    List<AiGenerationUsage> findAllWithMemberSince(@Param("from") LocalDateTime from);
