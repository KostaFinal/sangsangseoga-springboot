...
@AllArgsConstructor
@Builder
@Schema(description = "AI 이미지 생성 응답. Spring이 이미지를 영구 저장소에 저장한 뒤 최종 URL을 반환한다.")
public class AiGenerateImageResponseDto {
    ...
    @Schema(description = "생성된 이미지의 영구 URL. Replicate의 임시 delivery URL을 Spring이 즉시 다운로드해 " +
            "영구 저장소(app.storage.type에 따라 local/s3)에 저장한 뒤 서빙하는 절대 URL이다 - Replicate URL은 여기 담기지 않는다.")
